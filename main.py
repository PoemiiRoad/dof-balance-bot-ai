...
BUILD_VERSION = "2026.09.15-railway-daily-night-balance-v1"
...
class AccessMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: Message, data):
        ...
        try:
            ...
            try:
                await event.answer(
                    "⚠️ Не удалось выполнить команду. Повторите попытку. "
                    "Если ошибка повторяется, сообщите название кнопки и ответ /version.",
                    parse_mode=None,
                )
...
def _is_cancel_command(msg: Message) -> bool:
    ...
    if menu_action(msg) in {"суточный баланс", "ночная смена", "скользящий баланс"}:
        return True
...
async def handle_report(msg: Message, state: FSMContext):
    ...
    await wait_msg.edit_text(
        ...
        f"{last_day_note}"
        f"{warning_text}\n\n"
        f"Нажмите *📉 Скользящий баланс* для расчёта за 1, 2, 3 и 7 суток.",
        parse_mode="Markdown",
    )
...
async def report_sliding_balance(msg: Message, state: FSMContext):
    from balance_monitor import analyze, render
    await state.clear()
    active = db_get_active_report(msg.from_user.id)
    if not active:
        await msg.answer("Сначала загрузите отчёт.")
        return
    end = active_report_end_date(active)
    if end is None:
        await msg.answer("Нет завершённых суток для расчёта.")
        return
    # До 370 записей: окна через границу месяца и накопленная история.
    rows = db_get_latest_completed_days(370, end_date=end)
    try:
        text = render(analyze(rows, end, production_date(), WARN_PCT, CRIT_PCT))
    except (ValueError, TypeError, KeyError) as exc:
        await msg.answer(f"Контроль баланса недоступен: {exc}")
        return
    for chunk in split_message(text):
...
async def h_help(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        "📉 Скользящий баланс — небаланс за 1, 2, 3, 7 суток и накопленный итог\n"
