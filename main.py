...
async def _run_claude_agent(prompt: str) -> tuple[str, str | None, int | None]:
    """Один изолированный streaming-запрос к Claude без инструментов и памяти."""
    text_parts: list[str] = []
    ...
    if not answer and stream_parts:
...
async def report_monthly(msg: Message, state: FSMContext):
    ...
    text = (
        ...
        f"Конв.14: {fmt(s['kv14'])}т  ({pct_text(s['kv14'], s['kv4'])})\n"
        f"Конв.32: {fmt(s['kv32'])}т  ({pct_text(s['kv32'], s['kv4'])})\n"
        f"Конв.34: {fmt(s['kv34'])}т  ({pct_text(s['kv34'], s['kv4'])})\n"
        f"Конв.102: {fmt(s['kv102'])}т  ({pct_text(s['kv102'], s['kv4'])})\n"
        f"{em_bal(b1)} Баланс 1: {sign(b1)}\n"
        f"{em_bal(bc1)} Баланс С.1: {sign(bc1)}\n\n"
        f"━━ 2 очередь ━━\n"
        f"Конв.3: {fmt(s['kv3'])}т (осн.)   Конв.3Д: {fmt(s['kv3d'])}т (дубл.)\n"
        f"Конв.15: {fmt(s['kv15'])}т  ({pct_text(s['kv15'], s['kv3'])})\n"
        f"Конв.33: {fmt(s['kv33'])}т  ({pct_text(s['kv33'], s['kv3'])})\n"
        f"Конв.101: {fmt(s['kv101'])}т  ({pct_text(s['kv101'], s['kv3'])})\n"
        f"{em_bal(b2)} Баланс 2: {sign(b2)}\n"
        f"{em_bal(bc2)} Баланс С.2: {sign(bc2)}"
    )
